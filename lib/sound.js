let audioContext;
let diceRollBuffers = [];
let diceRollBuffersPromise;
let masterGain;

const diceRollAssetCandidates = [
  "/sounds/dice-roll.mp3",
  "/sounds/dice-roll-bouncy.mp3",
  "/sounds/dice-roll-cup.mp3",
...
async function loadDiceRollBuffers(context) {
  if (diceRollBuffers.length) return diceRollBuffers;
  if (diceRollBuffersPromise) return diceRollBuffersPromise;

  diceRollBuffersPromise = (async () => {
    const loadedBuffers = [];
    ...
    diceRollBuffers = loadedBuffers;
    return diceRollBuffers;
  })();

  return diceRollBuffersPromise;
}

function preloadDiceRollAsset(context) {
  loadDiceRollBuffers(context).catch(() => {});
}
...
function scheduleRecordedRoll(context, { diceCount, reroll }) {
  if (!diceRollBuffers.length) return false;
  ...
  const buffer = reroll
    ? diceRollBuffers.reduce((shortest, candidate) => (candidate.duration < shortest.duration ? candidate : shortest), diceRollBuffers[0])
    : diceRollBuffers[Math.floor(Math.random() * diceRollBuffers.length)];
  ...
  gain.gain.exponentialRampToValueAtTime(0.0001, startTime + Math.min(maxDuration, buffer.duration / playbackRate));
  ...
  source.stop(startTime + Math.min(maxDuration, buffer.duration / playbackRate));
  return true;
}
