...
export const defaultRules = [
  ...
  },
  {
    id: "rule-skill",
    title: "Skills",
    summary: "A useful skill gives advantage on the test.",
    details: "When a listed skill clearly helps the action, roll 4d6 instead of the normal 3d6.",
    tags: ["skill", "test"],
    pinned: true,
    open: false
  },
  {
    ...
    summary: "Positive trait use gives advantage; negative use can gift Furr-endship.",
    details: "When the trait helps, the character gains advantage. When it causes trouble, the table can gift one Furr-endship point to a comrade once per session.",
    tags: ["trait", "resource"],
    pinned: false,
    open: false
  },
  {
    id: "rule-cattribute",
    title: "Cattribute",
    summary: "Use the cattribute for an advantage, recovery, success, or other fitting benefit.",
    details: "A cattribute is a character-specific edge. The exact effect depends on the situation and the Storyteller's ruling.",
    tags: ["character", "test"],
    pinned: false,
    open: false
  },
  ...
  {
    id: "rule-catfight-actions",
    title: "Catfight Actions",
    summary: "Defend cancels successes, Help grants advantage, Hinder gives disadvantage.",
    details: "Non-aggressive options include Defend, Help, Move, Hinder, and Interact with scenery or objects. Kittens keep initiative while avoiding aggressive actions.",
    tags: ["catfight", "action"],
    pinned: false,
    open: false
  },
  {
...
export function normalizeCampaign(campaign) {
  const base = campaign && typeof campaign === "object" ? campaign : buildDefaultCampaign();
  const existingRules = Array.isArray(base.rules)
    ? base.rules.map((rule) => ({ ...rule, tags: Array.isArray(rule.tags) ? rule.tags : [] }))
    : [];
  ...
  return {
    ...base,
    schemaVersion: 1,
    characters: Array.isArray(base.characters)
      ? base.characters.map((character) => ({
          ...
          notes: character.notes ?? character.playerNotes ?? "",
          playerNotes: character.playerNotes ?? character.notes ?? "",
          storytellerNotes: character.storytellerNotes ?? ""
        }))
      : [],
    rules: [
      ...existingRules,
      ...defaultRules.filter((rule) => !existingRuleIds.has(rule.id)).map(cloneRule)
    ]
  };
}
